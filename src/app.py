...
with gr.Blocks(title="LLMLingua Demo", css=CSS, js=JS) as demo:
    ...
    with gr.Accordion("About this demo (please read):", open=False, elem_classes="accordion"):
        gr.Markdown(
            ...
            {'- Compression is done on a CPU. Using a GPU would be faster.' if not (MPS_AVAILABLE or CUDA_AVAILABLE) else '- Compression is done on a GPU using MPS.' if MPS_AVAILABLE else f'- Compression is done on {torch.cuda.get_device_name()}.'}
            ...
            - Token counts are calculated based on the `cl100k_base` [tokenizer](https://platform.openai.com/tokenizer) (GPT-3.5/-4) and may vary for different target models. Saving metric is based on an API pricing of $0.06 / 1000 tokens.
